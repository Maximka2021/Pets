  ...
  <button className={isShow ? 'not-moved' : 'moved'}  onClick={() => setIsShow(!isShow)} >{isShow ?"⬅️" :"➡️"}</button>
    <div className={isShow ? "form-hide" : "form-show"}>
      <form className="form" onSubmit={handleClick}>
        <h1 className="form-title">Pet Form</h1>
        ...
        <input onChange={(e)=> setAge(e.target.value)}  className="form-input" placeholder="Age"></input> 
        <textarea onChange={(e) => setDescription(e.target.value)}  className="form-input" placeholder="Description" rows="7" cols="30" /> 
